...
class InferenceCore:
    def __init__(self, prop_net: PropagationNetwork, images, num_objects, mem_freq=1):
        ...
        self.low_feat = torch.zeros(
            (t, 256, nh // 4, nw // 4), dtype=torch.float32, device=self.device
        )
        self.prob[0] = 1e-7
    ...
    def do_pass(self, key_k, key_v, idx, end_idx):
        ...
        for ti in this_range:
            ...
            f16, f8, f4, k16, v16 = self.get_query_kv_buffered(ti)
            self.low_feat[ti] = f4
            out_mask= self.prop_net.segment_with_query(
                this_k, this_v, f16, f8, f4, k16, v16
            )
    ...
    def interact(self, mask, frame_idx, end_idx):
        ...
        self.label_idx.append(frame_idx)
        # KV pair for the interacting frame
        label_img = self.images[:, frame_idx].cuda()
        key_k, key_v = self.prop_net.memorize(
            label_img, self.prob[1:, frame_idx].cuda()
        )

        # additionally generate low_level value feature for label frame
        self.low_feat[frame_idx] = self.low_level_extractor(label_img)
