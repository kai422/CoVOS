...
class Decoder(nn.Module):
    ...
    def forward(self, f16, f8, f4):
        ...
        out = F.interpolate(x, scale_factor=4, mode="bilinear", align_corners=False)
        return out
    ...
    def segment(self, frame, keys, values, selector=None):
        ...
        if self.single_object:
            ...
            logits = torch.cat(
                [
                    self.decoder(
                        self.memory(keys[:, 0], values[:, 0], k16, v16), f8, f4
                    ),
                    self.decoder(
                        self.memory(keys[:, 1], values[:, 1], k16, v16), f8, f4
                    ),
                ],
                1,
            )
