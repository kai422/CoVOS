...
def evaluate_dataset(dset, keyframe_dict, results_path, measure='J', to_file=True):
    ...
    if to_file:
        if keyframe_dict:
            f = open(results_path / ("evaluation_keyframes-%s.txt" % measure), "w")
        else:
    ...
    for j, sequence in enumerate(dset):
        ...
        if keyframe_dict:
            keyframe = keyframe_dict[seq_name]
        else:
            keyframe = None

        r = utils.evaluate_sequence(segmentations, annotations, object_info, keyframe, measure=measure)
        results[seq_name] = r
